...
\begin{center}
  \includegraphics[width=0.6\textwidth]{cascaded_tanks.png}
\end{center}
...
* Among several valid options, we choose the model structure:
  ...
  \begin{align*}
  x(k+1) &= x(k) + W_2 \tanh(W_1 \mathrm{vec}(x(k), u(k)) + b_1) + b_2 \\
  y(k) &=  C x (k)
  \end{align*}
...
* We will implement simulation error minimization:
   ...
   \begin{equation*}
     \hat \param, \hat {x}(0) = \arg \min_{\param, x(0)} \frac{1}{T} \sum_{k=0}^{T-1}  \| {y^\mathrm{sim}(k) - y(k)}\|^2
   \end{equation*}

  * $y^\mathrm{sim}(k)$: output *simulated* by iterating the model, starting from $x(0)$
  * Loss minimized w.r.t. the parameters $\param$ and the initial state $x(0)$
