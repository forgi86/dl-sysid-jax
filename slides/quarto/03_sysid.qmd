...
subtitle: "Fitting neural state-space models"
...
* $\param \in \mathbb{R}^{n_\param}$ is the parameter vector

No need to convince you that they are general and powerful!
\pause

\vskip 1em
* We will see how to fit state-space models to data using JAX
* $f, g$ represented as neural networks.
...
\begin{center}
  \includegraphics[width=0.6\textwidth]{img/cascaded_tanks.png}
\end{center}

A pretty common [**benchmark**](https://www.nonlinearbenchmark.org/benchmarks/cascaded-tanks) for system identification algorithms.
...
* Among several valid options, we choose the model structure:
  ...
  \begin{align*}
  x(k+1) &= x(k) + W_2 \tanh(W_1 [x(k)^\top\;; u(k)^\top]^\top + b_1) + b_2 \\
  y(k) &=  C x (k)
  \end{align*}
...
* We will implement simulation error minimization:
   ...
   \begin{equation*}
     \hat \param, \hat {x}(1) = \arg \min_{\param, x(1)} \frac{1}{T} \sum_{k=1}^{T}  \| {y^\mathrm{sim}(k) - y(k)}\|^2
   \end{equation*}

  * $y^\mathrm{sim}(k)$: output *simulated* by iterating the model, starting from $x(1)$
  * Loss minimized w.r.t. the parameters $\param$ and the initial state $x(1)$
...
plt.title(f"Training: {iters} Adam iterations, learning rate {lr}")
plt.plot(LOSS)
...
```
Not too bad! Check out the official [**Benchmark Results**](https://www.nonlinearbenchmark.org/benchmarks/cascaded-tanks)
...
* Try out different models. You may modify:
  ...
  * Output equation as another feedforward neural net
  * ...
\vskip 1em
...
\vskip .5em
* Re-implementation of existing SYSID methods from the literature: https://github.com/forgi86/jax-ident
\vskip .5em
* Same in PyTorch: https://github.com/forgi86/pytorch-ident
\vskip .5em
* PyTorch SYSID package from TU/e: https://github.com/MaartenSchoukens/deepSI
\vskip .5em
...
* I seldom write neural network models and parameter initialization code from scratch
   * Libraries like `flax`, `haiku`, and `eqinox` may be convenient
   * They also also offers an object-oriented interface similar to PyTorch -->