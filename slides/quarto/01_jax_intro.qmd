...
\pause
* Apply $f$ with "true" $p=p^o = [1.0\;\; 2.0]$ input data point $x = 0.5$
...
ax[1].plot(x, f(p_hat, x), "g", label=r"$f(\hat{p}^{200}, x)$")
ax[1].plot(x, f(p_init, x), "b", label=r"$f(\hat{p}^{1}, x)$")
ax[1].legend()
plt.tight_layout()
```

## Exercises
* Open the accompanying notebook and familiarize with the environment/code
